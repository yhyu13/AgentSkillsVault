...
Implement `<aisides-project>` through small, independently reviewed increments.
...
- **Forward-slash workspace paths on Windows bash.** When creating Kanban cards from bash (git-bash/MSYS), backslash paths like `<aisides-project>` get eaten into `D:GitRepo-MyAISidesProject` (non-absolute), causing `spawn_failed` and auto-archive after 2 failures. ALWAYS use forward slashes: `--workspace "dir:<aisides-project>"`. If a card is already stuck with a broken path (status will be `archived`), recreate the full chain with forward-slash paths.
