...
3. **为什么 / 验证哪条能力** — 架构类必写，**紧接 TL;DR，正文第一块**。这篇设计是为了压哪条运行时承诺（插件接缝、模型可见⟺已落日志、不改 loop…）。不是待办说明书。配一张探针图。不要先铺错路再补为什么。
4. **错路对照** — 读者的第一反应往往是错的。先画错路，再画正路。开口的那句主线可以落在这里。
...
- 标题下第一块就是 **TL;DR**。架构类第二块就是 **为什么 / 验证哪条**。然后才错路。
